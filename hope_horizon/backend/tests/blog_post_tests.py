...
class BlogPostTests(APITestCase):
    ...
    def test_list_blog_post_pagination_page_number_greater_than_total_pages(self):
        ...
        self.assertEqual(response.data["page_information"]["actual_size"], 0)
    ...
    def test_list_blog_post_group_name(self):
        ...
        response = self.client.get(f"/api/blog-post?group_name={self.test_group.name}&owned=true&page=1&blog_post_type_id=3", format="json", follow=True)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data["blog_posts"]), 1)
    ...
    def test_list_blog_post_group_name_not_owned(self):
        ...
        self.client.logout()
        self.client.login(username="testuser2", password="testpassword")
        response = self.client.get(f"/api/blog-post?group_name={self.test_group.name}&owned=false&page=1&blog_post_type_id=3", format="json", follow=True)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data["blog_posts"]), 1)
    ...
    def _validate_data_list_pagination(self, data, page=1, page_size=10):
        ...
        self.assertEqual(data["page_information"]["actual_size"], len(data["blog_posts"]))
